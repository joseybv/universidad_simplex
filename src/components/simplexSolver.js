...
var tooBigString =
  "Too many matrices in your expression," +
  cr +
  "or your expression is too complicated." +
  cr +
...
var theInstructionString =
  ...
  cr +
  "     necessarily in the constraints). " +
  cr +
  tab +
  tab +
  "    (eg. p = 0x + 2y + 0z ) " +
...
function SetupTableau() {
  ...
  if (check == -1) {
    ...
    document.theSpreadsheet.output.value =
      "That does not look like a linear programming problem to me!" +
      cr +
      cr +
      "Press Example to see how to type one in.";
    okToRoll = false;
    return 666;
  }
...
function displayFinalStatus() {
  ...
  if (TableauNumber > maxSteps)
    document.theSpreadsheet.expr.value =
      "No optimal solution found after 50 steps. Aborted.";
  else if (singular)
    document.theSpreadsheet.expr.value =
      "No optimal solution exists for this problem.";
  else {
    document.theSpreadsheet.expr.value =
      "Optimal Solution: " + objectiveName + " = ";
...
function doIt() {
  ...
  // Option 2 // preliminary checks
    ...
    if (accuracydig == "" || !looksLikeANumber(accuracydig)) {
      document.theSpreadsheet.expr.value =
        "Enter a value for the accuracy (Rounding) in the range 1-13.";
      okToRoll = false;
    }
    ...
    if (okToRoll) {
      ...
      if (thenum < 1 || thenum > 14) {
        document.theSpreadsheet.expr.value =
          "Accuracy (Rounding) must be in the range 1-13.";
        okToRoll = false;
      } else numSigDigs = thenum;

      if (document.theSpreadsheet.input.value == "") {
        document.theSpreadsheet.expr.value =
          "Enter a linear programming problem above (or press Example).";
        okToRoll = false;
      }
    } // if okToRoll
  } // end of this option
  ...
  //
    ...
    document.theSpreadsheet.expr.value =
      "Press 'Solve' to solve the given problem.";
